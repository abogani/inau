...
@app.task(name='inau.build.cleanup_old_builds')
def cleanup_old_builds(days: int = 90):
    """Task periodico per pulire le build vecchie"""
    cutoff_date = datetime.utcnow() - timedelta(days=days)
    
    with Session(engine) as session:
        old_builds = session.exec(
            select(Build).where(
                Build.date < cutoff_date,
                Build.status.in_([BuildStatus.SUCCESS, BuildStatus.FAILED])
            )
        ).all()
        
        cleaned = 0
        for build in old_builds:
            # Artifacts vengono eliminati automaticamente grazie a ON DELETE CASCADE
            session.delete(build)
            cleaned += 1
            
        session.commit()
        logger.info(f"Cleaned up {cleaned} old builds")
        
    return {"cleaned": cleaned}

# Configurazione Celery Beat per task periodici
app.conf.beat_schedule = {
    'cleanup-old-builds': {
        'task': 'inau.build.cleanup_old_builds',
        'schedule': 86400.0,  # Una volta al giorno
        'args': (90,)  # Mantieni builds degli ultimi 90 giorni
    },
}
